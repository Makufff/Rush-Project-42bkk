...
addEventListener('scroll',()=>{
  if(spyTick) return;
  spyTick=requestAnimationFrame(()=>{spy();spyTick=0;});
},{passive:true});
...
spy();activateByHash(location.hash||'#home');

const sparkles=$('#sparkles'),icons=['✨','💙','💖','🌸','⭐'];
function addSpark(){
...
const stage=$('.stage'),bokehEl=$('#bokeh');
...
const ig=$('.ig'),char=$('.char');let parallaxOn=true;
...
stage.addEventListener('pointerleave',()=>{
  ig.style.transform='';char.style.transform='';stage.style.backgroundPosition='';
});

function confetti(x,y){
  const n=24,frag=document.createDocumentFragment();
  for(let i=0;i<n;i++){
...
const likeBtn=$('#likeBtn'),likeCountEl=$('#likeCount'),likesStat=$('#likesStat'),igFrame=$('#igFrame');
let likes=Number(localStorage.getItem('likes')||5000),liked=localStorage.getItem('liked')==='1';
const fmtNum=n=>n>=1e9?(n/1e9).toFixed(1)+'B':n>=1e6?(n/1e6).toFixed(1)+'M':n>=1e3?(n/1e3).toFixed(1)+'k':String(n);
function renderLikes(){likeCountEl.textContent=fmtNum(likes);likesStat.textContent=fmtNum(likes);likeBtn.classList.toggle('liked',liked);likeBtn.setAttribute('aria-pressed',liked)}
renderLikes();
function floatHeart(x,y){
  const h=document.createElement('div');
  h.className='float-heart';h.style.setProperty('--x',x+'px');h.style.setProperty('--y',y+'px');h.textContent='❤';
  ig.appendChild(h);setTimeout(()=>h.remove(),900);
}
function toggleLike(e){
  const r=ig.getBoundingClientRect();
  if(!liked){liked=true;likes+=1;floatHeart(e?e.clientX-r.left:r.width/2,e?e.clientY-r.top:r.height/2)}
  else{liked=false;likes=Math.max(0,likes-1)}
  localStorage.setItem('likes',likes);localStorage.setItem('liked',liked?'1':'0');renderLikes();
}
likeBtn.addEventListener('click',toggleLike);
igFrame.addEventListener('dblclick',toggleLike);

const comments=$('#comments'),cBtn=$('#commentBtn'),cInput=$('#commentInput'),cList=$('#commentList'),toast=$('#toast');
...
$$('[data-copy]').forEach(btn=>btn.addEventListener('click',async()=>{
  try{await navigator.clipboard.writeText(btn.getAttribute('data-copy'));showToast('Copied: '+btn.getAttribute('data-copy'))}catch{}
}));

const revealEls=document.querySelectorAll('.xp-card,.skills-card,.card-proj,.c-card,.ig,.char');
revealEls.forEach(el=>{el.style.opacity=0;el.style.transform='translateY(18px)'});
...
(function(){
  ...
  async function load(){
    ...
    try{
        ...
        for(const u of rawCandidates){
          try{
            ...
            try{
              ...
              if(r.ok){const j=await r.json();xml=j&&j.contents?j.contents:''}
            }catch{}
            if(!xml){
      ...
      if(data.feed?.link){headLink.href=data.feed.link;headLink.style.display='inline'}
      feedEl.innerHTML='';
      (data.items||[]).slice(0,3).forEach(it=>{
        ...
        const a=node.querySelector('.thumb'),img=node.querySelector('img');
        const thumb=it.thumbnail||pickThumb(it)||'';
        if(thumb){img.src=thumb;img.alt=it.title||'';a.href=it.link}else a.remove();
